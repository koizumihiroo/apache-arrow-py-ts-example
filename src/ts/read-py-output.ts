...
function showTableMarkdown(t: Table): string {
  ...
  const headers = fields
    .map(
      (x) =>
        `${x.name}<br>${x.type}${x.nullable === true ? '<br>nullable' : ''}`
    )
    .join(" | ")
    .addStartEnd("|");
  const spliter = fields
    .map(() => " --- ")
    .join("|")
    .addStartEnd("|");
  const values = Array.from(Array(t.length), (_, v) => v)
    .map((i) => t.get(i).toArray().join(' | ').addStartEnd('|'))
    .join('\n');
  const out = [headers, spliter, values].join('\n') + '\n';
  return out;
}

const arrow1 = readFileSync('data/df.feather');
const table1 = Table.from(arrow1);
console.log(showTableMarkdown(table1));

const arrow2 = readFileSync('data/df.arrow');
